  ...
  blocks_(blocks), object_uri_(service)
{
  ...
  //  ROS_INFO("----%f , %f \n", x,y);
  //  ROS_INFO("----%f , %f \n", origin_offset_x_,origin_offset_y_);
}
...
bool TileLoader::insideCentreTile(double lat, double lon) const
{
  ...
  ROS_INFO("**** %f , %f \n", lat, lon);
  ROS_INFO("----%f , %f \n", x, y);
  ROS_INFO("center_tile %d , %d \n", center_tile_x_, center_tile_y_);
...
void TileLoader::start()
{
  ...
  ROS_INFO("min_x: %d  min_y: %d  max_x: %d  max_y: %d ", min_x, min_y, max_x, max_y);
  //  initiate requests
